...
INNER JOIN amazon_profile ON scr_u_id=profile_id and scr_u_id=".$this->user_id);
       ...
       if(!empty($res))
       {
          ...
          //$this->email->from('ravi.amzon@gmail.com');
          $this->email->from('feedback_mail@proseller.in');
          $this->email->to("payment@proseller.in");
...
INNER JOIN amazon_profile ON scr_u_id=profile_id and scr_u_id=".$this->user_id);
    ...
    if(!empty($res))
    {
      ...
      $this->email->initialize($config);
      //$this->email->from('ravi.amzon@gmail.com');
      $this->email->from('yugandhar@lemertech.com');
      $this->email->to("yugandhar@lemertech.com");
      // $this->email->to('griffith.dickins@gmail.com');
      $this->email->subject($subject);
      $this->email->message($msg);
      $this->email->send();
    }
