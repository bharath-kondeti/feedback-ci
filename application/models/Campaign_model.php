...
class Campaign_model extends CI_Model
{
 ...
 public function get_brand_list($country_code='')
  ...
  public function get_product_list($country_code='',$brand_code='',$key_word='',$fc_code='',$offet = '',$limit = '')
  {
  ...
  public function get_product_count($country_code='',$brand_code='',$key_word='',$fc_code='',$offet = '',$limit = '')
  {
    $sql="SELECT count(*) as total_count from customer_product WHERE store_id=".$this->store_id." AND is_active=1 AND prod_brand<>''";
    if(!empty($key_word))
    {
    $key_word=str_replace("'","\'",$key_word);
      $sql.=" AND (prod_sku=".$this->db->escape($key_word)." OR prod_asin=".$this->db->escape($key_word)." OR prod_title LIKE '%".$key_word."%')";
