...
$conf_gold_plan = array(
	...
	'interval_count' => '1',
	'price' => '299',
	'trial' => '+30 days', // OR +3 minutes => Add 3 Days For Trial
	'num_ringtones' => '3'
);
...
class Stripe_lib {
	...
	function charge($paymentTokenClient) {
		$charge = \Stripe\Charge::create([
			...
			'receipt_email' => 'yugandhar@lemertech.com',
	...
	function getSubscriptionProduct($data = NULL) {
		if($data) {
	...
	function getallPlan($data = NULL) {
		if($data) {
			...
			return "[ Stripe_lib => getallPlan() ] => Please Configure Your Subscription Plan => read https://stripe.com/docs/billing/subscriptions/products-and-plans";
		}
	}
	
	function getSubscriptionList($data = NULL) {
		if($data) {
			$plan = \Stripe\Subscription::all([
				'limit' => $data['limit'],
				//'nickname' => $data['plan_nickname'], // A brief description of the plan, hidden from customers.
				//'interval' => $data['plan_interval'], //'month', // day, week, month, and year
				//'interval_count' => $data['plan_interval_count'], //'1', // 1
			...
			return "[ Stripe_lib => getSubscriptionList() ] => Please Configure Your Subscription Plan => read https://stripe.com/docs/billing/subscriptions/products-and-plans";
		}
	}
	...
	function deleteWebhook($idSubscription = NULL) {
		// imposta come default alla funzione un id nullo, verifica quindi che l'ID sia sempre presente prima di inviare la richiesta
		if($idSubscription) {
			$subscription = \Stripe\WebhookEndpoint::retrieve($idSubscription);
			$subscription->delete(); // cancella subito la sottoscrizione
			/* OPPURE CANCELLA LA SOTTOSCRIZIONE ALLA FINE DEL PERIODO CORRENTE
			$subscription->cancel_at_period_end = true;
			$subscription->save();
			*/
	...
	function createWebHook($data = NULL) {
		if($data) {
			$subscription = \Stripe\WebhookEndpoint::create([
				'url' => $data['url'],
			...
			return "[ Stripe_lib => createWebHook() ] => Configure Subscription Error => https://stripe.com/docs/billing/quickstart";
		}
		
	}


function getWebhookList($data = NULL) {
		if($data) {
			$plan = \Stripe\WebhookEndpoint::all([
				'limit' => $data['limit'],
				//'nickname' => $data['plan_nickname'], // A brief description of the plan, hidden from customers.
				//'interval' => $data['plan_interval'], //'month', // day, week, month, and year
				//'interval_count' => $data['plan_interval_count'], //'1', // 1
	...
	function verifyingSignaturesWebHook($payload, $sig_header) {
		$endpoint_secret = 'whsec_p3HQq9Hfn1agifddAKZPeSuxaC3Pzlce';
		$event = null;
		try {
	...
	function updateSubscription($data = NULL) {
		if($data) {
			$subscription = \Stripe\Subscription::update('sub_GqXD930cCjoNdI',[
				'trial_end' => 'now',
			]);
			return $subscription;
		} else {
			return "[ Stripe_lib => updateSubscription() ] => Configure Subscription Error => https://stripe.com/docs/billing/quickstart";
		}
		
	}
	
	function createCharge($data = NULL) {
		if($data) {
